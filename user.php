...
elseif ($action == 'act_account')
{
	...
	if ($currency_type == 1){
		$amount = isset($_POST['amount_cny']) ? floatval($_POST['amount_cny']) : 0;
	}
	else{
    ...
    else
    {
		...
		include_once(ROOT_PATH . 'languages/zh_cn/common.php');
		if($surplus['currency_type'] == 1){
			$order['surplus_amount'] = round($amount/floatval($xe),2);
		}
		else{
			$order['surplus_amount'] = $amount;
		}
        ...
        //计算此次预付款需要支付的总金额
		if($surplus['currency_type'] == 1){
			$order['order_amount'] = round($amount/floatval($xe),2) + $payment_info['pay_fee'];
		}
		else{
        ...
        if($surplus['currency_type'] == 0){
			...
			$amount_in_cad = round($amount/floatval($xe),2);
			$smarty->assign('amount',  '人民币' . strval($amount) . '元' . ' (CAD$' . strval($amount_in_cad) . ')');
		}
        $smarty->assign('order',   $order);
        $smarty->display('user_transaction.dwt');
    }
...
elseif ($action == 'act_edit_surplus')
{
	...
	include_once(ROOT_PATH . 'languages/zh_cn/common.php');
	$surplus_cny_cvt = round(floatval($surplus_cny)/floatval($xe),2);
