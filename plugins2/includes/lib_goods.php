...
{ 
	...
	if (!make_dir(ROOT_PATH.$dir.'/'.$sub_dir.'/use_img'))
    {
        return false;
    }
    switch($type)
    {
        ...
        case 'gallery_thumb':
            ...
            break;
		case 'use_img':
            $img_name = $goods_id . '_use_U_' . $rand_name;
            break;
    }
    if ($position == 'source')
    {
    ...
    elseif ($position == 'use')
    { 
        if (move_image_file(ROOT_PATH.$source_img, ROOT_PATH.$dir.'/'.$sub_dir.'/use_img/'.$img_name.$img_ext))
        {
            return $dir.'/'.$sub_dir.'/use_img/'.$img_name.$img_ext;
        }
    }
    else
    { 
