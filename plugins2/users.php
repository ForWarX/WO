...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    if (!$users->add_user2($username, $password, $email))
    {
        /* 插入会员数据失败 */
    ...
    {
        log_account_change($_SESSION['user_id'], 0, 0, 0, $GLOBALS['_CFG']['register_points'], $GLOBALS['_CFG']['register_points'], $_LANG['register_points']);
    }
...
{
	$filename = $_CFG['shop_name'] . "会员列表";
	header("Content-type: application/vnd.ms-excel; charset=utf-8");
    header("Content-Disposition: attachment; filename=$filename.xls");
	$sql = "SELECT user_name, mobile_phone, email FROM " .$ecs->table('users');
	$chaxun = $db->query($sql);
	$data = $filename . "\t\n";
	$data .= $_LANG['label_user_name'] . "\t";
	$data .= $_LANG['mobile_phone'] . "\t";
	$data .= $_LANG['email'] . "\t\n";
	while ($val = $db->fetchRow($chaxun))
    {
		$data .= $val['user_name'] . "\t";
        $data .= $val['mobile_phone'] . "\t";
		$data .= $val['email'] . "\t\n";
	}
	if (EC_CHARSET != 'gbk')
    {
...
function user_list()
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT user_id, user_name, email, is_validated, user_money, user_money_cny, frozen_money, rank_points, pay_points, reg_time ".
