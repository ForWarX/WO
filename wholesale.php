...
if ($_REQUEST["act"] == "submit_user")
{
	$email_to = "store@pbcc.ca";
	$email_subject = "B2B会员申请";
	
	if (!empty($_POST['name']) && !empty($_POST['company']) && !empty($_POST['position']) && !empty($_POST['email']) && !empty($_POST['demand'])){
	...
	$email_message .= "姓名: ".$name."\n";
 	$email_message .= "公司名称: ".$company."\n";
	$email_message .= "职位: ".$position."\n";
    $email_message .= "Email: ".$email."\n";
    $email_message .= "求购商品: ".$demand."\n";
	...
	show_message('提交成功！我们会在3个工作日内审核您的申请。', '回到首页', 'index.php');
	}
	}
...
{
    //show_message($_LANG['ws_user_rank'], $_LANG['ws_return_home'], 'index.php', 'info',false);
	show_message($_LANG['ws_user_rank'], '', '', 'info', false);
}
...
if ($_REQUEST['act'] == 'list')
{
    ...
    if ($count > 0)
    {
        ...
        $display  = in_array($display, array('list', 'text')) ? $display : 'list';
...
elseif ($_REQUEST['act'] == 'submit_order')
{
	...
	/* 订单信息 */
    $smarty->assign('order',      $order);
	$smarty->assign('order_amount', price_format($order['order_amount'], false));
