...
$cache_id = sprintf('%X', crc32($cat_id . '-' . $display . '-' . $sort  .'-' . $order  .'-' . $page . '-' . $size . '-' . $_SESSION['user_rank'] . '-' .
	...
	$dwt_template = 'category.dwt';
	if ($cat['is_event_area'] == 1 && $cat['event_file_name'] != ''){
		
		//生成专区的custom文件
		$event_php_folder = ROOT_PATH;		

		$event_template_folder = 'themes/' . $_CFG['template'];
		$event_php_file   = $event_php_folder . $cat['event_file_name'] . '.php';
		...
		$event_js_file   = $event_template_folder . '/js/' . $cat['event_file_name'] . '.js';
		...
		//转向刚生成的专区php
		header('Location:' . $cat['event_file_name'] . '.php');
	}	
...
if (!$smarty->is_cached('category.dwt', $cache_id))
{
	...
	/*子分类商品分开显示*/
	$child_tree = get_child_tree($cat_id);
	
	$cated_listing = array();
	foreach($child_tree as $single_cat){
		$cated_listing[$single_cat['id']]['id'] = $single_cat['id'];
		$cated_listing[$single_cat['id']]['name'] = $single_cat['name'];
		$cated_listing[$single_cat['id']]['url']  = build_uri('category', array('cid' => $single_cat['id']), $single_cat['name']);
		
		$subcat_children = get_children($single_cat['id']);
		$subcat_goodslist = category_get_goods($subcat_children, $brand, $price_min, $price_max, $ext, $size, $page, 'goods_number', 'DESC');
		$cated_listing[$single_cat['id']]['list'] = $subcat_goodslist;
		$cated_listing[$single_cat['id']]['kj_instock'] = get_category_recommend_goods('hot', $single_cat['id']);
	
	}
	
	if(!empty($cated_listing)){
		$smarty->assign('cated_listing',    $cated_listing);
	}
	
	$banners = get_catpagebanner_xml($cat_id);
	$smarty->assign('banners',          $banners);  // 轮换banner
...
{
    return $GLOBALS['db']->getRow('SELECT cat_name, keywords, cat_desc, style, grade, filter_attr, parent_id, is_event_area, event_file_name FROM ' . $GLOBALS['ecs']->table('category') .
        " WHERE cat_id = '$cat_id'");
}
...
function category_get_goods($children, $brand, $min, $max, $ext, $size, $page, $sort, $order)
{
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ' .
                ...
                'g.promote_start_date, g.promote_end_date, g.goods_brief, g.goods_thumb , g.goods_img, g.is_shipping ' .
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
		...
		$arr[$row['goods_id']]['goods_name']       = remove_ks($row['goods_name']);
...
function get_catpagebanner_xml($cat_id)
{
    $flashdb = array();
    if (file_exists(ROOT_PATH . DATA_DIR . '/catpagebanner/'.$cat_id.'.xml'))
    {

        // 兼容v2.7.0及以前版本
        if (!preg_match_all('/item_url="([^"]+)"\slink="([^"]+)"\stext="([^"]*)"\ssort="([^"]*)"/', file_get_contents(ROOT_PATH . DATA_DIR . '/catpagebanner/'.$cat_id.'.xml'), $t, PREG_SET_ORDER))
        {
            preg_match_all('/item_url="([^"]+)"\slink="([^"]+)"\stext="([^"]*)"/', file_get_contents(ROOT_PATH . DATA_DIR . '/catpagebanner/'.$cat_id.'.xml'), $t, PREG_SET_ORDER);
        }
