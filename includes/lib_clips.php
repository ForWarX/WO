...
function insert_user_account($surplus, $amount)
{
    $sql = 'INSERT INTO ' .$GLOBALS['ecs']->table('user_account').
           ' (user_id, admin_user, currency_type, amount, add_time, paid_time, admin_note, user_note, vip_duration, process_type, payment, is_paid, pay_points)'.
            " VALUES ('$surplus[user_id]', '', '$surplus[currency_type]', '$amount', '".gmtime()."', 0, '', '$surplus[user_note]', '$surplus[vip_duration]', '$surplus[process_type]', '$surplus[payment]', 0, '$surplus[pay_points]')";
    $GLOBALS['db']->query($sql);
...
function get_user_default($user_id)
{
    ...
    $sql = "SELECT pay_points, user_money, user_money_cny, credit_line, last_login, is_validated, vip_start, vip_end FROM " .$GLOBALS['ecs']->table('users'). " WHERE user_id = '$user_id'";
    $row = $GLOBALS['db']->getRow($sql);
    $info = array();
    $info['username']  = stripslashes($_SESSION['user_name']);
    $info['shop_name'] = $GLOBALS['_CFG']['shop_name'];
    $info['integral']  = "<span style='color:#EF5A2C;font-weight:bold;'>" . $row['pay_points'] . "</span> " . $GLOBALS['_CFG']['integral_name'];
    /* 增加是否开启会员邮件验证开关 */
	...
	/* 高级会员:开通日 和 有效期 */
	$info['vip_start'] = $row['vip_start'];
	$info['vip_end'] = $row['vip_end'];
	...
	$info['surplus_cny'] = 'RMB￥' . strval($row['user_money_cny']); 
...
function get_comment_list($user_id, $page_size, $start)
{
    /*$sql = "SELECT c.*, g.goods_name AS cmt_name, r.content AS reply_content, r.add_time AS reply_time ".
           ...
           " WHERE c.user_id='$user_id' AND c.parent_id=0";
    $res = $GLOBALS['db']->SelectLimit($sql, $page_size, $start);*/
	
	
	
	$sql = "SELECT c.*, g.goods_name AS cmt_name ".
           " FROM " . $GLOBALS['ecs']->table('comment') . " AS c ".
           " LEFT JOIN " . $GLOBALS['ecs']->table('goods') . " AS g ".
           " ON c.comment_type=0 AND c.id_value = g.goods_id ".
           " WHERE c.user_id='$user_id' AND c.parent_id=0 ORDER BY c.add_time";
    $res = $GLOBALS['db']->SelectLimit($sql, $page_size, $start);
	...
	$comments = get_comment_like_and_img($comments);

	

	foreach ($comments as $key => $value){
		
	$temp_comment_id = $value['comment_id'];
    $sql = "SELECT * ".
           " FROM " . $GLOBALS['ecs']->table('comment').
           " WHERE parent_id='$temp_comment_id' ORDER BY add_time";
    $res = $GLOBALS['db']->query($sql);
		while ($row = $GLOBALS['db']->fetchRow($res))
		{
			//$row['formated_add_time'] = local_date($GLOBALS['_CFG']['time_format'], $row['add_time']);
...
function get_comment_like_and_img($comment){
	//showr($comment);
