...
function order_fee($order, $goods, $consignee)
{
	...
	/*汇率*/
	$xe = floatval(get_exchange_rate("CNY","CAD"));
	
	if ($total['amount'] > 0)
    {
        if (isset($order['surplus_cny']) && round($order['surplus_cny']/$xe,2) > $total['amount'])
        {
            $order['surplus_cny'] = round($total['amount']*$xe,1);
            $total['amount']  = 0;
        }
        else
        {
            $total['amount'] -= floatval(round($order['surplus_cny']/$xe,2));
        }
    }
    else
    {
