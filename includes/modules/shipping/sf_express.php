    ...
    function calculate($goods_weight, $goods_amount, $goods_number)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
					@$step_1 = $this->configure['step_fee'];
					$step_2 = $step_1 + 0.05;
					$weight_less_5 = $lb_goods_weight - 5;
					$int_test_2 = (int)$weight_less_5;
					$diff = $weight_less_5 - $int_test_2;
					$diff_100 = $diff * 100;
					$int_diff_100 = (int)$diff_100;
					if (($lb_goods_weight) > 5 && ($lb_goods_weight) <= 50)
					{
						if ($int_diff_100 > 1)
						{
							$fee += ceil($weight_less_5) * $step_1;
						}
						else
						{
							$fee += $weight_less_5 * $step_1;
						}
						
					}
					else if (($lb_goods_weight) > 50)
					{
						if ($int_diff_100 > 1)
						{
							$fee += ceil($lb_goods_weight) * $step_2 - 7;
						}
						else
						{
							$fee += $lb_goods_weight * $step_2 - 7;
						}
