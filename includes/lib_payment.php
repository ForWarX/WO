        ...
        if ($pay_log && $pay_log['is_paid'] == 0)
        {
            ...
            elseif ($pay_log['order_type'] == PAY_SURPLUS)
            {
                ...
                if(empty($res_id))
                {
					...
					if($arr['currency_type'] ==0){
						if (intval($arr['amount']) == 20){
							$amount_with_bonus = 20.2;	
						}
						else if (intval($arr['amount']) == 50){
							$amount_with_bonus = 51.5;
						}
						else{
							$amount_with_bonus = 105;
						}
					}
					else{
						if (intval($arr['amount']) == 100){
							$amount_with_bonus = 101;	
						}
						else if (intval($arr['amount']) == 500){
							$amount_with_bonus = 515;
						}
						else{
							$amount_with_bonus = 1050;
						}	
					}
                    log_account_change($arr['user_id'], $arr['currency_type'], $amount_with_bonus, 0, 0, 0, $_LANG['surplus_type_0'], ACT_SAVING);
                }
            }
