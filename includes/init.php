...
require(ROOT_PATH . 'includes/global_val.php');
...
$trans_cur = "CAD";
$cur_date = time();
$sql = "SELECT * FROM " . $ecs->table('xe_rate') . " WHERE base_cur = '" . $base_cur ."' AND trans_cur = '" . $trans_cur ."'";
$row = $db->GetRow($sql);
include(ROOT_PATH . 'includes/xe.php');
if(!empty($row)){
	if($row['cur_date'] < $cur_date){
		if($row['cur_date'] < 1000000000){
			$next_date = gmmktime(3,5,0,date("m", $cur_date),date("d", $cur_date),date("Y", $cur_date)) + 86400;
		}
		else{
			$next_date = intval($row['cur_date']) + 86400;
		}
		$rate = get_xe_rate(date("Y-m-d", $cur_date),$base_cur,$trans_cur);
		$sql = "UPDATE " . $ecs->table('xe_rate') . 
		" SET rate = " . $rate . ", cur_date = " . $next_date .
		" WHERE base_cur = '" . $base_cur ."' AND trans_cur = '" . $trans_cur ."'";
		$db->query($sql);
	}
}
else{
	$next_date = gmmktime(3,5,0,date("m", $cur_date),date("d", $cur_date),date("Y", $cur_date)) + 86400;
	$rate = get_xe_rate(date("Y-m-d", $cur_date),$base_cur,$trans_cur);
	$sql = "INSERT INTO " . $ecs->table('xe_rate') . " (base_cur, trans_cur, cur_date, rate) VALUES ('" . $base_cur ."','" . $trans_cur . "'," . $next_date ."," . $rate .")"; 
	$db->query($sql);
}
if(!defined('INIT_NO_SMARTY')){
//$smarty->assign('exchange_rate', get_exchange_rate($base_cur,$trans_cur));}
$smarty->assign('exchange_rate', 100);}
?>