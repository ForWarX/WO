...
{
    /* 查询订单信息，检查状态 */
    $sql = "SELECT user_id, order_id, order_sn , surplus, surplus_cny , integral , bonus_id, order_status, shipping_status, pay_status FROM " .$GLOBALS['ecs']->table('order_info') ." WHERE order_id = '$order_id'";
    ...
    if ($GLOBALS['db']->query($sql))
    {
        ...
        if ($order['user_id'] > 0 && $order['surplus'] > 0)
        {
            ...
            log_account_change($order['user_id'], 0, $order['surplus'], 0, 0, 0, $change_desc);
        }
		if ($order['user_id'] > 0 && $order['surplus_cny'] > 0)
        {
            $change_desc = sprintf($GLOBALS['_LANG']['return_surplus_on_cancel'], $order['order_sn']);
            log_account_change($order['user_id'], 1, $order['surplus_cny'], 0, 0, 0, $change_desc);
        }
        if ($order['user_id'] > 0 && $order['integral'] > 0)
        {
            $change_desc = sprintf($GLOBALS['_LANG']['return_integral_on_cancel'], $order['order_sn']);
            log_account_change($order['user_id'], 0, 0, 0, 0, $order['integral'], $change_desc);
        }
...
function drop_consignee($id)
{
    ...
    else
    {
        ...
        $path_to_id_front = 'data/idimg/'.$uid .'/'.$id.'f.jpg';
		if(file_exists($path_to_id_front)){
			@unlink($path_to_id_front);
		}
		
		$path_to_id_back = 'data/idimg/'.$uid .'/'.$id.'b.jpg';
		if(file_exists($path_to_id_back)){
			@unlink($path_to_id_back);
		}
...
function deleteRepeat($array){
  ...
  *
  */
function upload_id($upload, $side, $ref, $image){
	
	$sign = ($side == 'front') ? 'f' : 'b';
	
	if (isset($upload['error'])) // php 4.2 版本才支持 error
    {
        // 最大上传文件大小
        ...
        if ($upload['error'] == 0)
        {
            ...
            {
                return array(false, "文件格式不正确，请重新上传");
            }
        }
        elseif ($upload['error'] == 1)
        {
            return array(false, "文件太大，请重新上传");
        }
	...
	// 如果上传了商品图片，相应处理
    if (($upload['tmp_name'] != '' && $upload['tmp_name'] != 'none'))
    {
		if ($ref['user_id'] > 0)
        {
            /* 删除原来的图片文件 */
            $path_to_img = 'data/idimg/'.$ref['user_id'] .'/'.$ref['address_id']. $sign.'.jpg';
			if(file_exists($path_to_img)){
				@unlink($path_to_img);
			}
			$id_img   = $image->upload_image($upload,'idimg/'.$ref['user_id'],$ref['address_id']. $sign.'.jpg'); // 原始图片
			
			if ($id_img === false)
			{
				return array(false, $image->error_msg());
			}
			return array(true, "");
        }
	}
