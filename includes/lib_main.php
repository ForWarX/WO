...
function update_user_info()
{
    ...
    /* 查询会员信息 */
    $time = date('Y-m-d');
    $sql = 'SELECT u.user_money,u.email, u.pay_points, u.user_rank, u.rank_points, u.vip_end, '.
...
function get_user_info($id=0)
{
    ...
    $time = date('Y-m-d');
    $sql  = 'SELECT u.user_id, u.email, u.user_name, u.user_money, u.pay_points, u.user_rank'.
            ' FROM ' .$GLOBALS['ecs']->table('users'). ' AS u ' .
            " WHERE u.user_id = '$id'";
        ...
        if (in_array($filename, array('category', 'goods', 'article_cat', 'article', 'brand')))
        {
            ...
            if (!empty($cat_arr))
            {
                ...
                foreach ($cat_arr AS $val)
                {
                    ...
                    $sql = "SELECT parent_id FROM " . $GLOBALS['ecs']->table("category") . " WHERE cat_id = " . $val['cat_id'];
...
function license_info()
{
    if($GLOBALS['_CFG']['licensed'] > 0)
    {
        ...
        $license = '<a href="http://www.pbcc.ca/license.php?product=ecshop_b2c&url=' . urlencode($host) . '" target="_blank"
>&nbsp;&nbsp;Licensed</a>';
        return $license;
    }
    else
...
function select_top_liked_comments($arr,$limitNum = 3){
		
		$top_liked_arr = array();
		$count_top_liked = count($arr);
		$counter = 0;
		foreach ($arr as $value){
			$counter++;
			$top_liked_arr[] = $value;
			if ($count_top_liked <= $limitNum){
				if ($counter == $count_top_liked){
					break;
				}
			}
			else{
		...
		$temp = array();
		foreach($top_liked_arr as $key => $value){
			$sql ="SELECT * FROM " . $GLOBALS['ecs']->table('comment'). 
			" WHERE comment_id = ". $value['comment_id'];
			
			$res = $GLOBALS['db']->selectLimit($sql, 1);

			$ids = '';
			$row = $GLOBALS['db']->fetchRow($res);
						
				$ids .= $ids ? ",$row[comment_id]" : $row['comment_id'];
				$temp[$row['comment_id']]['id']       = $row['comment_id'];
				$temp[$row['comment_id']]['email']    = $row['email'];
				$temp[$row['comment_id']]['username'] = $row['user_name'];
				$temp[$row['comment_id']]['content']  = str_replace('\r\n', '<br />', htmlspecialchars($row['content']));
				$temp[$row['comment_id']]['content']  = nl2br(str_replace('\n', '<br />', $temp[$row['comment_id']]['content']));
				$temp[$row['comment_id']]['parent_id']= $row['parent_id'];
				$temp[$row['comment_id']]['rank']     = $row['comment_rank'];
				$temp[$row['comment_id']]['add_time'] = local_date($GLOBALS['_CFG']['time_format'], $row['add_time']);
				$temp[$row['comment_id']]['user_id']  = $row['user_id'];
				$temp[$row['comment_id']]['is_admin'] = $row['is_admin'];
				$temp[$row['comment_id']]['top']      = '1';
				...
				if ($ids)
				{
					...
					{     	$counter++;
							...
							$temp[$row['parent_id']]['children'][$counter]['content']  = nl2br(str_replace('\n', '<br />', htmlspecialchars($row['content'])));
							$temp[$row['parent_id']]['children'][$counter]['add_time'] = local_date($GLOBALS['_CFG']['time_format'], $row['add_time']);
							$temp[$row['parent_id']]['children'][$counter]['email']    = $row['email'];
							$temp[$row['parent_id']]['children'][$counter]['username'] = $row['user_name'];
							$temp[$row['parent_id']]['children'][$counter]['user_id']  = $row['user_id'];
							$temp[$row['parent_id']]['children'][$counter]['is_admin'] = $row['is_admin'];
		...
		$top_liked_arr = $temp;$temp = array();
		return $top_liked_arr;
}
